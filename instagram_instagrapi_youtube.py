#!/usr/bin/env python3
"""
Instagram to YouTube using Instagrapi
Fastest and most reliable Instagram library
Works on Railway!
"""
import os
import sys
...
class InstagramDownloader:
    def __init__(self):
        self.temp_dir = BASE_DIR / 'temp_instagram_api'
        self.temp_dir.mkdir(exist_ok=True)
        
        self.tracking_file = BASE_DIR / 'instagram_api_processed.json'
    ...
    def _login(self):
        ...
        if INSTAGRAM_USERNAME and INSTAGRAM_PASSWORD:
            try:
                ...
                print(f"⚠ Challenge required - Instagram security check")
                print(f"→ Will try without login (public accounts only)")
            except Exception as e:
                print(f"⚠ Login failed: {e}")
                print(f"→ Will try without login (public accounts only)")
        else:
            print("→ No credentials provided")
            print("→ Will access public accounts without login")
    ...
    def download_from_account(self, username):
        """Download latest posts from Instagram account using Instagrapi"""
        try:
    ...
    def monitor(self):
        print(f"\n{'='*60}")
        print(f"Instagram to YouTube - Instagrapi")
        print(f"{'='*60}")
        print(f"Using: Instagrapi (Fastest library)")
        ...
        for username in INSTAGRAM_ACCOUNTS:
            print(f"  - @{username}")
        print(f"\nFeatures:")
        print(f"  ✓ Fast API access: ENABLED")
