#!/usr/bin/env python3
"""
Instagram to YouTube Downloader - LOCAL VERSION
Optimized for running on your Windows PC with home internet
100% Working - No Railway issues!
"""
import os
import sys
import time
...
# Configuration
INSTAGRAM_USERNAME = 'rebel_jallal'  # Your Instagram login
INSTAGRAM_PASSWORD = 'RebelJallal123'  # Your Instagram password

# Accounts to download from (can be your own or others if public)
...
class InstagramDownloader:
    ...
    def _login(self):
        ...
        try:
            ...
            print("  4. Instagram security check required")
            sys.exit(1)
    ...
    def download_from_account(self, username):
        """Download latest posts from Instagram account"""
        try:
            ...
            print(f"  → Restart the script to login again")
        except Exception as e:
            print(f"  ✗ Error: {e}")
    
    def monitor(self):
        print(f"\n{'='*60}")
        print(f"Instagram to YouTube Downloader - LOCAL VERSION")
        print(f"{'='*60}")
        print(f"Running on: Windows PC (Home Internet)")
        print(f"Check interval: {CHECK_INTERVAL//60} minutes")
        print(f"YouTube: @LahoriTwins")
        print(f"\nLogged in as: @{INSTAGRAM_USERNAME}")
        print(f"\nDownloading from:")
        for username in DOWNLOAD_FROM_ACCOUNTS:
            print(f"  - @{username}")
        print(f"\nFeatures:")
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("Instagram to YouTube - LOCAL VERSION")
    print("="*60 + "\n")
